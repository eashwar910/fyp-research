...
Model routing: subagents run on Sonnet (`model: sonnet` in each `agents/*.json`). You run on whatever the owner launched you with. Do not downgrade the gate stages to a subagent.
